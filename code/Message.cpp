...
【更改记录】
*************************************************************************/
Message Message::MakeStopAns(){
    return Message(MSG_STOP, nullptr, 0);
}

/*************************************************************************
【函数名称】Message::FromPopQuiz
【函数功能】产生一个随机提问消息
【参数】aString 输入参数 要转换的字符串
【返回值】随机提问消息对象
【开发者及日期】陈文泽(cwz19@mails.tsinghua.edu.cn) 2020-06-19
【更改记录】
*************************************************************************/
Message Message::FromPopQuiz(const string& aString) {
    return Message(MSG_POPQUIZ, (const unsigned char*)aString.c_str(), aString.length());
}

/*************************************************************************
【函数名称】Message::FromQuiz
【函数功能】产生一个问题消息
【参数】aQuiz 要转换的消息
【返回值】随机提问消息对象
【开发者及日期】陈文泽(cwz19@mails.tsinghua.edu.cn) 2020-06-19
【更改记录】
*************************************************************************/
Message Message::FromQuiz(const StudentQuiz& aQuiz){
    unsigned int optNum = aQuiz.optNum;
    unsigned int usetime = aQuiz.m_uiUseTime;
    vector<string> opts;
    aQuiz.get_opts(opts);
    vector<bool> corrans = aQuiz.m_CorrAns;
    vector<bool> myans = aQuiz.m_MyAns;
    //计算总长
    unsigned int totalLen = sizeof(unsigned int) * 3 + aQuiz.Question.length();
    for(unsigned int i = 0; i < optNum; i++){
        totalLen += opts[i].length();
        totalLen += sizeof(int) * 2;
    }
    unsigned char* Buffer = new unsigned char[totalLen];
    totalLen = 0;
    //选项个数
    memcpy(Buffer, (char*)&optNum, sizeof(unsigned int));
    totalLen += sizeof(unsigned int);
    //答题时间
    memcpy(Buffer + totalLen, (char*)&usetime, sizeof(unsigned int));
    totalLen += sizeof(unsigned int);
    //问题描述
    unsigned int len = aQuiz.Question.length();
    memcpy(Buffer + totalLen, (char*)&len, sizeof(unsigned int));
    totalLen += sizeof(unsigned int);
    memcpy(Buffer + totalLen, aQuiz.Question.c_str(), len);
    totalLen += len;
    //选项描述、我的答案、正确答案
    for(unsigned int i = 0; i < optNum; i++){
        len = opts[i].length();
        memcpy(Buffer + totalLen, (char*)&len, sizeof(unsigned int));
        totalLen += sizeof(unsigned int);
        memcpy(Buffer + totalLen, opts[i].c_str(), len);
        totalLen += len;
        int tempBool = (corrans[i] == true ? 1 : 0);
        memcpy(Buffer + totalLen, (char*)&tempBool, sizeof(int));
        totalLen += sizeof(int);
        tempBool = (myans[i] == true ? 1 : 0);
        memcpy(Buffer + totalLen, (char*)&tempBool, sizeof(int));
        totalLen += sizeof(int);
    }
    Message ret(MSG_QUIZ, Buffer, totalLen);
    delete [] Buffer;
    return ret;
}
...
【更改记录】
*************************************************************************/
Message Message::FromPerformance(const Performance& aPer){
    string name = aPer.m_sName;
    unsigned int alltime = aPer.m_uiAllTime;
    unsigned int concenTime = aPer.m_uiConcenTime;
    unsigned int NameLen = name.length();
    unsigned char* Buffer = new unsigned char[NameLen  + sizeof(unsigned int) * 3];
    memcpy(Buffer, (char*)&NameLen, sizeof(unsigned int));
    memcpy(Buffer + sizeof(unsigned int), name.c_str(), NameLen);
    memcpy(Buffer + sizeof(unsigned int) + NameLen, (char*)&alltime, sizeof(unsigned int));
    memcpy(Buffer +  2 * sizeof(unsigned int) + NameLen, (char*)&concenTime, sizeof(unsigned int));
    Message ret(MSG_PERFOR, Buffer, NameLen + sizeof(unsigned int) * 3);
    delete [] Buffer;
    //qDebug() << "from performance" << QString::fromStdString(aPer.name);
    return ret;
}
...
/*************************************************************************
【函数名称】Message::ToPerformance
【函数功能】从学生表现消息还原一个学生表现对象
【参数】
    Msg 入口参数，要还原的消息
    aPer出口参数 学生表现对象
【返回值】无
...
【更改记录】
*************************************************************************/
void Message::ToPerformance(const Message& Msg, Performance& aPer){
    //开辟临时缓冲区
    unsigned char* pBuffer = new unsigned char[Msg.Size];
    //消息类型和解码后长度临时变量
    MessageType type;
    ...
    if (!IsDecoded || Msg.Type() != MSG_PERFOR) {
        delete[] pBuffer;
        throw(invalid_argument("Message type is not MSG_PERFOR"));
    }
    //封装学生姓名
    unsigned int LenName = 0;
    memcpy((char*)&LenName, pBuffer, sizeof(unsigned int));
    string Name = string((char*)pBuffer + sizeof(unsigned int), LenName);
    //封装总时长
    unsigned int alltime;
    memcpy((char*)&alltime, pBuffer + sizeof(unsigned int) + LenName, sizeof(unsigned int));
    //封装专注时常
    unsigned int concentime;
    memcpy((char*)&concentime, pBuffer + sizeof(unsigned int) * 2 + LenName, sizeof(unsigned int));
    //释放临时缓冲区
    aPer = Performance(Name, alltime, concentime);
    delete[] pBuffer;
}

/*************************************************************************
【函数名称】Message::ToPopQuiz
【函数功能】从随机提问消息还原一个文本对象
【参数】入口参数，要还原的消息
【返回值】文本对象
【开发者及日期】陈文泽(cwz19@mails.tsinghua.edu.cn) 2020-06-19
【更改记录】
*************************************************************************/
string Message::ToPopQuiz(const Message& Msg){
    //开辟临时缓冲区
    unsigned char* pBuffer = new unsigned char[Msg.Size];
    //消息类型和解码后长度临时变量
    MessageType type;
    ...
    bool IsDecoded = Msg.Decode(type, pBuffer, length);
    //解码失败或者不是string消息，抛出异常
    if (!IsDecoded || Msg.Type() != MSG_POPQUIZ) {
        delete[] pBuffer;
        throw(invalid_argument("Message type is not MSG_POPQUIZ"));
    }
    //形成文本
    string res = string((char*)(pBuffer), length);
    //释放临时缓冲区
...
【更改记录】
*************************************************************************/
void Message::ToQuiz(const Message& Msg, StudentQuiz& aQuiz){
    unsigned char* pBuffer = new unsigned char[Msg.Size];
    MessageType type;
    unsigned int length;
    bool IsDecoded = Msg.Decode(type, pBuffer, length);
    if (!IsDecoded || Msg.Type() != MSG_QUIZ) {
        delete[] pBuffer;
        throw(invalid_argument("Message type is not MSG_QUIZ"));
    }
    //准备容器
    unsigned int optNum;
    ...
    //选项个数
    memcpy((char*)&optNum, pBuffer, sizeof(unsigned int));
    totalLen += sizeof(unsigned int);
    //答题时间
    memcpy((char*)&usetime, pBuffer + totalLen, sizeof(unsigned int));
    totalLen += sizeof(unsigned int);
    //问题描述
    unsigned int len;
    memcpy((char*)&len, pBuffer + totalLen, sizeof(unsigned int));
    totalLen += sizeof(unsigned int);
    question = string((char*)pBuffer + totalLen, len);
    totalLen += len;
    //选项描述、我的答案、正确答案
    for(unsigned int i = 0; i < optNum; i++){
        memcpy((char*)&len, pBuffer + totalLen, sizeof(unsigned int));
        totalLen += sizeof(unsigned int);
        string temps = string((char*)pBuffer + totalLen, len);
        totalLen += len;
        opts.push_back(temps);
        int tempBool;
        memcpy((char*)&tempBool, pBuffer + totalLen, sizeof(int));
        totalLen += sizeof(int);
        corrans.push_back(tempBool == 1 ? true : false);
        memcpy((char*)&tempBool, pBuffer + totalLen, sizeof(int));
        totalLen += sizeof(int);
        myans.push_back(tempBool == 1 ? true : false);
    }
    aQuiz = StudentQuiz(question, opts,corrans, usetime, myans);
    delete[] pBuffer;
}
