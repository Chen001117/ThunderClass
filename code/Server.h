...
【更改记录】
    ...
    2020-05-15 由范静涛增加注释
    2020-06-19 由陈文泽增加sendtorand函数
    2020-06-20 由陈文泽将send等三个函数改为const
*************************************************************************/
class Server
{
public:
    ...
    bool SendTo(unsigned int idx, const Message& aMessage)const;

    //群发消息
    void SendToAll(const Message& aMessage)const;

    //像随机用户发消息
    void SendToRand(const Message& aMessage)const;
