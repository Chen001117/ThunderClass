...
/*************************************************************************
【函数名称】Admin::delUser
【函数功能】按用户名、密码、用户类型删除一个用户对象，对象的指针保存在基类User的静态对象vector<User*> UserList中
【参数】三个参数均为输入参数，分别为用户名、密码、类型（Admin、Teacher、Student三者之一）
【返回值】true表示删除成功，false表示删除失败
【开发者及日期】范静涛(fanjingtao@tsinghua.edu.cn) 2020-5-10
【更改记录】2020-5-15 由范静涛增加注释
*************************************************************************/
bool Admin::delUser(const string &Name, const string &Password, const string &Type){
    //
    return User::delUser(Name, Password, Type);
}
