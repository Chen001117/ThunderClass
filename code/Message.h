...
【接口说明】
    ...
    静态成员函数FromPoPQuiz产生一个随机提问消息
    静态成员函数ToString从文本消息产生一个字符串
    静态成员函数ToPopQuiz从随机提问消息产生一个字符串
...
【更改记录】
    2020-05-10 由范静涛做了命名规范化修改
    2020-05-15 由范静涛增加注释
    2020-06-19 由陈文泽添加popquiz相关两个静态函数
    2020-06-19 由陈文泽添加quiz相关两个静态函数
    2020-06-20 由陈文泽添加stopAns函数
*************************************************************************/
class Message{
public:
    //构造函数
    ...
    //构造随机提问消息
    static Message FromPopQuiz(const string& aString);
    //构造学生表现消息
    static Message FromPerformance(const Performance& aPer);
    //构造发题消息
    static Message FromQuiz(const StudentQuiz& aQuiz);
    ...
    //还原随机提问消息
    static string ToPopQuiz(const Message& Msg);
    //还原语音消息
    ...
    //还原学生表现消息
    static void ToPerformance(const Message& Msg, Performance& aPer);
    //还原发题目消息
    static void ToQuiz(const Message& Msg, StudentQuiz& aQuiz);
