...
【更改记录】
    ...
    2020-05-15 由范静涛增加注释
*************************************************************************/
bool Server::SendTo(unsigned int idx, const Message& aMessage)const {
...
【更改记录】
    ...
    2020-05-15 由范静涛增加注释
*************************************************************************/
void Server::SendToAll(const Message& aMessage)const{
    //遍历每个连接，依次发送出去
...
/*************************************************************************
【函数名称】Server::SendToRand
【函数功能】像随机客户端发送消息
【参数】入口参数，要发送的消息
【返回值】无
【开发者及日期】陈文泽(cwz19@mails.tsinghua.edu.cn) 2020-06-19
【更改记录】
    2020-06-20 由陈文泽将srand函数移出while回圈
*************************************************************************/
void Server::SendToRand(const Message& aMessage)const{
    //如果没有学生，不做任何事
    ...
    //若该连接没人，重抽
    while(isSend == false){
        unsigned int random = rand() % m_Connections.size();
        isSend = SendTo(random, aMessage);
    }
}
